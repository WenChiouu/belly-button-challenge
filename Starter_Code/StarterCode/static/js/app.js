...
function demoInfo(sample)
{
    ...
    d3.json("samples.json").then((data) => {
        ...
        // use Object.entires to get the value key pairs
...
function initialize()
{
    ...
    d3.json("samples.json").then((data) => {
        ...
        // use a foreach in order to create options for seach samples in the
        // selector
        sampleNames.forEach((sample) => {
            select.append("option")
                .text(sample) // what you see on the drop down - applying text
                .property("value", sample); // defines - corresponds to sample - establish when calling a sample
        });
...
{
    // call the update to the medatdata
    demoInfo(item);
    // call function to build the bar chart
    buildBarChart(item);
    // call function to build the bar chart
